...
Never use: "should work", "I think", "probably", "seems to be", "I believe",
"looks like it works", "that should do it", "functionally complete".
...
Only for: throwaway prototypes, pure one-liner rewrites, docs-only, config-only.
Must acknowledge: `"prove-it suspended — run [cmd] before shipping."`

## Modes

- `verify` (default) — plan + output
- `tdd` — red → green (same test)
- `strict` — compile ✓ + regression ✓ + new tests ✓ + edges ✓
